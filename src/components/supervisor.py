    ...
    def _create_devices(self):
        ...
        for user_identity in userCredentials:
            ...
            for i in range(self._ndevices):
                ...
                devName = self._device_kwargs["device_name"] + str(j)
                deviceInfo[devName] = {'delay': delay, 'servicePath': '/{}'.format(devName)}
                j = j+1
                device.setDaemon(True)
                device.start()
                self._devices.append(device)
        # move device delays in json file
        with open('/var/lib/cert-storage/deviceInfo.json', 'w') as fp:
            json.dump(deviceInfo, fp)
